...
#define cursor_window   IniParams[5].xy
#define cursor_hotspot  IniParams[5].zw
#define window_size     IniParams[6].xy
#define cursor_showing  IniParams[6].z
#define cursor_pass     IniParams[6].w
...
void main(out vs2ps output, uint vertex : SV_VertexID)
{
    ...
    //output.pos.x += userIPDShift * GUIDepth;
    output.pos.x += GUIDepth > 1 ? userIPDShift + halfVirtualIPDShift : userIPDShiftGUI * GUIDepth;
}
#endif /* VERTEX_SHADER */
...
void main(vs2ps input, out float4 result : SV_Target0)
{
	...
	result = cursor_color.Load(float3(input.texcoord, 0)); //arma3 cursor_color have alpha so just load it
}
#endif /* PIXEL_SHADER */
